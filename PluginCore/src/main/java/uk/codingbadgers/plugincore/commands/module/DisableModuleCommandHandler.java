...
public class DisableModuleCommandHandler implements ICommandHandler {
    ...
    DisableModuleCommandHandler(ModuleLoader moduleLoader, File moduleFile) {
    ...
    public void handle(MessageSystem messageSystem, CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!module.isEnabled()) {
            messageSystem.SendMessage(sender, module.getName() + " is already disabled.");
            return;
        }

        module.setEnabled(false);
        messageSystem.SendMessage(sender, "Disabled " + module.getName());
    }
}
